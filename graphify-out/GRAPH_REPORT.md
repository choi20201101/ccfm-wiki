...
## Corpus Check
- Large corpus: 426 files · ~230,539 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 237 nodes · 310 edges · 29 communities detected
...
### Community 28 - "루솔브 퍼포먼스 캔버스 코드"
Cohesion: 1.0
Nodes (1): 루솔브 퍼포먼스 캔버스 v1 → xlsx (바탕화면 출력)

## Knowledge Gaps
- **74 isolated node(s):** `QSCV docx → raw/qscv/ 원본 md + chunk/ 200줄 청크 분할 - 표 셀도 텍스트로 추출 - 이미지/도형은 [IMG] 플`, `Step 01b (optional) — detect face in after.png and save face_box.json.  Uses O`, `Step 05 — thumbnails + captions + zip.`, `Step 02 — Kling image2video: JWT, submit, poll, download, resume.`, `Shared helpers: config loading, path resolution, ffmpeg runner.` (+69 more)
