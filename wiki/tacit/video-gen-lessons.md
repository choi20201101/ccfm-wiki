...
## §43 — Omni Flash vs Veo 3.1 모델 분기 룰 (i2v, 2026-05-29)

**언제 적용**: Google Flow 안에서 i2v 컷을 만들 때 어느 모델을 쓸지 결정해야 할 때. [[sources/src-omni-vs-veo31-flow-2026-05-28]] 7케이스 비교에서 추출.

### 분기 룰 (decision-rule)

| 컷 성격 | 1순위 | 이유 |
|---|---|---|
| 뷰티 비포애프터 / 피부 클로즈업 | **Omni** | 피부 톤 자연, 벗겨내는 표현 자연 |
| 인물 모션·말하기·한국어 발화 | **Omni** | 의도 추론 강, 말투 자연 |
| 제품 클로즈업 / 브랜디드 컨셉 | **Omni** | 원본 디테일 유지 + 무드 자동 |
| 시작·종료 프레임 고정 필요 (엔딩 로고 인, 스토리보드 정합) | **Veo 3.1** | Omni는 키프레임 컨트롤 불가 |
| 공간 이동 / B-roll / 트랜지션 | **Veo 3.1** | 공간 표현 자연 |
| 대량 시안 (크레딧 절감) | **Veo 3.1** | 0 크레딧 (Omni는 25/8초) |

### 2-패스 크레딧 절감 패턴 ⭐
1. **Veo 3.1로 5~10개 드래프트 대량 생성** (0 크레딧) → 컨셉 채택
2. **채택된 컷만 Omni로 파이널 리테이크** (25 크레딧 × N)
3. 결과: 단일 Omni 풀생성 대비 약 70% 크레딧 절감

### 프롬프트 전처리 차이 (자동화 시 LLM 분기)
- **Omni**: 의도 중심 짧은 프롬프트 OK (40~80 단어). subject + key_action + mood
- **Veo**: 프레임-바이-프레임 디테일 강제 (120~250 단어). start_state + action_beats + camera_path + end_state

### 공통 약점 (재생성 가드레일)
- 드론 워킹 등 복잡한 카메라 무빙 → 둘 다 부자연
- 음식 베어무는 소리·소품 물리 (그릇 공중부양 등) → 후반 SFX 필요, QA 게이트로 자동 검출 → Omni 재생성

### 광고 1편(15~20초 릴스) 권장 컷 분배
| 컷 | 모델 | 크레딧 |
|---|---|---|
| 비포애프터 / 인물 후킹 | Omni | 25 |
| 제품 클로즈업 | Omni | 25 |
| B-roll / 트랜지션 | Veo 3.1 | 0 |
| 엔딩 로고 인 (종료프레임) | Veo 3.1 | 0 |

→ 평균 50~75 크레딧/편.

- confidence: medium (1차 head-to-head 후기 기반, 2~3편 추가 검증 시 high로 승격)
- source: [[sources/src-omni-vs-veo31-flow-2026-05-28]] (3팀 도혜진/김성웅 후기)
- related: §41 Seedance/Veo3/Sora2 보이스 일관성, [[sources/src-gemini-omni-guide]] (Omni 사용법)
