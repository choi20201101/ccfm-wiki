...
### 함정/제약
- **비밀번호 하드코딩**: `auth/login.py`, `auth/save_cookies.py`, `join/join_cafe.py`, `join/batch_join.py` 에 `mkm202601 / wnstjr66^^` 가 plaintext로 박혀 있음 → 유출 위험. **리팩토링 필요**.
...
- 파일: `auth/login.py` `wait_for_answer()`
...
- **하드코딩된 크레덴셜**: `auth/login.py`, `auth/save_cookies.py`, `join/join_cafe.py`, `join/batch_join.py` 4개 파일에 `mkm202601 / wnstjr66^^` plaintext → 유출 경로.
...
**출처:** `C:\Users\Administrator\Desktop\market-research-package\modules\cafe\` (2026-04-13 기준 `src/auth/login.py`, `src/join/join_cafe.py`, `src/join/batch_join.py`)
...
## [2026-04-15] Gemini는 preserved-face 인물의 body 부피 증가를 하드 리밋

출처: goglecc 15회 반복 (v4 ~ v14) → [[src-goglecc-seed-curation]] 섹션 10.

### 관찰
얼굴을 보존하는 reference가 있으면 Gemini는 해당 인물의 body를 **살찌게** 못 만듦.
다음 우회 시도 모두 실패 (재현 일관성 high):
- 전신 "+40kg 고도비만" 극단 프롬프트
- 3단 체인 (+5kg, +10kg, +15kg 각각 호출)
- 국소 변경 ("노출된 배 부분만 살 붙여")
- 2-image blending (얼굴 ref + 통통 몸 ref)
- 한국어/영어/"과거 사진" framing

### 조건
- Gemini 2.5 Flash Image (`google-genai` 직호출, `gemini-2.5-flash-image`)
- Preserved-identity 인물이 있으면 발동
- 반대 방향(살빼기)은 OK — Gemini는 slim-down 허용

### 해법
- **Face-swap 후처리**: 몸(통통) + 얼굴(예쁨) 각각 생성 후 `fal-ai/face-swap`으로 합성
- fal endpoint 필드: `base_image_url`(몸), `swap_image_url`(얼굴)
- 대안: `easel-ai/advanced-face-swap`은 `gender_0`, `workflow_type:"target_hair"` 필수

### confidence: high (15회 독립 시도에서 모두 동일 거부 패턴 재현)

## [2026-04-15] Higgsfield Soul API 실전 스펙

### 관찰
Higgsfield Soul 공식 엔드포인트 — SDK 코드 역분석으로 확인:
- Base: `https://platform.higgsfield.ai`
- **Auth 헤더**: `hf-api-key` + `hf-secret` (Authorization 헤더 아님 — 주의)
- SoulID 생성: `POST /v1/custom-references` → body `{name, input_images:[{type:"image_url", image_url}]}` → status 폴링 `GET /v1/custom-references/{id}` ("completed" 대기, ~5분)
- 이미지 생성: `POST /higgsfield-ai/soul/standard` → body `{prompt, aspect_ratio, resolution:"1080p" 또는 "720p", custom_reference_id, custom_reference_strength}` → 폴링 `GET /requests/{id}/status`
- resolution은 `"1080p"` 정확히 (`"hd"` 같은 값 422)

### SoulID 학습 요령
- 10~20장 **같은 스타일/미학**이면 됨 (같은 사람일 필요 X)
- 예: K-pop 연예인 10명 → "K-pop idol 톤" 학습 (generation마다 다른 K-pop 얼굴 변주)
- 학습 데이터의 평균으로 회귀 → 슬림만 학습하면 "살찐 K-pop 얼굴" 못 뽑음 (generation은 ordinary chubby로 drift)

### strength 튜닝
- 0.85 = 얼굴 보존 강함, 체형 변화 제한
- 0.5~0.6 = 체형 자유 변화 허용, 얼굴은 학습 분포에서 변주
- 1.0 = 완전 학습 분포 복원
- confidence: high
...
### [2026-05-12] Gemini CLI는 자동화 파이프라인에 부적합 — SDK 직호출이 정답
관찰 (오늘의유머 마이닝 파이프라인 1469건 자동 처리 중):
- **CLI는 "도우려는 에이전트"로 동작** → 구조화된 마크다운 템플릿 프롬프트를 무시하고 "이미지를 정리해 드립니다", "파일을 생성하시겠어요?" 같은 사용자-친화 응답 생성. plan 모드(`--approval-mode plan`) + 영어 명령형 헤더(`REPLY WITH ONLY THE MARKDOWN BELOW`)로 bash 직접 호출 시엔 성공했으나, 동일 prompt를 Python `subprocess.run([gemini.cmd, ...])`로 호출하면 또 다시 채팅 모드로 빠짐.
- **CWD 의존성**: 폴더에 이전 실행의 `idea.md`가 남아 있으면 "이미 했네" 판단 → 빈 응답 또는 "이미 저장되어 있는 것을 확인했습니다"
- **Windows .cmd shim**: subprocess가 cmd shim을 통해 호출할 때 quoting/escape가 bash 호출과 다르게 동작. 같은 prompt가 환경에 따라 결과가 갈림
- **출력 인코딩 변동**: `--output-format text`에도 mojibake가 일부 케이스에서 발생

→ **자동화는 무조건 `google.genai` SDK 직호출**. `genai.Client()` + `types.Part.from_bytes(data=p.read_bytes(), mime_type=...)`로 이미지 첨부, `GenerateContentConfig(temperature=0.9, top_p=0.92, max_output_tokens=4800)`. `max_output_tokens=2400`은 6섹션 템플릿에 부족 — 중간에 잘림 발견.

조건: Vision + 구조화 출력 모두 필요한 자동화 파이프라인. 단발 사람-인터랙션(쉘에서 한 번 물어보고 끝)은 CLI도 무방.
- source: 2026-05-12 picosera/new/todayhumor_ideas 파이프라인 디버깅. CLI 4회 시도(plan 모드, output-format text, prompt 재작성, idea.md 사전 삭제) 모두 실패 후 SDK 전환 즉시 성공.
- confidence: high (반복 검증 4회 CLI 실패 → SDK 1회 즉시 성공, 이후 614건 안정 처리)
- cross-ref: [[domains/todayhumor-mining-playbook]] §Gemini CLI vs SDK

---

### [2026-05-12] Windows Git Bash + nohup/&는 신뢰 X — schtasks 1회 발화로 wrap
관찰:
- `nohup python ... > out.log 2>&1 &` 가 Git Bash on Windows에서 즉시 exit code 2로 종료 (nohup 자체가 PATH에 없거나 인식 안 됨). 출력 파일도 생성 안 됨
- 동일 명령을 Python `subprocess.Popen` 대신 foreground 실행하면 정상 동작
- 결국 **백그라운드 작업/스케줄 1회 발화는 Windows schtasks가 정답**

```cmd
cmd.exe /c "schtasks /Create /TN <Name> /SC ONCE /SD 2026/05/12 /ST 09:00 /TR \"C:\path\to\resume.bat\" /F"
```
⚠️ Git Bash에서 `schtasks /Create /TN ...` 직접 호출하면 `/F`, `/TN` 등을 `/C:/Program Files/Git/F` 식으로 path 해석해 실패. **반드시 `cmd.exe /c "..."` 래퍼**로 통째 전달.

.bat 파일에는 `cd /d "C:\절대경로"` 첫 줄에 필수 (schtasks 실행 시 CWD 보장 안 됨).

- source: 2026-05-11~12 picosera 자동 재개 셋업
- confidence: high (3회 시도, schtasks 방식만 안정)
- cross-ref: [[domains/todayhumor-mining-playbook]] §운영 노하우

---

### [2026-05-12] 오늘의유머 리스트 파서 — `tr.list_tr_humordata` + `td.subject a`만 사용
관찰:
- 한 행에 `view.php` 앵커가 3개 이상 (번호 셀 / 제목 셀 / 댓글 셀 / 작성자 셀). 전역 `soup.select('a[href*=view.php]')`로 잡으면 번호 셀의 숫자 텍스트를 제목으로 잡는 버그 발생 → 인덱스 전체가 mojibake/숫자로 채워짐
- ✅ 정답 선택자:
  ```python
  for tr in soup.select("tr.list_tr_humordata"):
      title = tr.select_one("td.subject a").get_text(strip=True)
      no    = re.search(r"no=(\d+)", tr.select_one("td.no a")["href"]).group(1)
      views = int(tr.select_one("td.hits").get_text(strip=True).replace(",", ""))
      recs  = int(tr.select_one("td.oknok").get_text(strip=True).replace(",", ""))
  ```
- 인코딩: `r.encoding = r.apparent_encoding or "utf-8"` 미적용 시 한글 mojibake → 정규식 hard-drop 필터 전부 빗나감
- view 페이지 title은 신뢰 X (사이드바·관련글 셀렉터 충돌) — **list crawl 단계의 title을 끝까지 들고 갈 것**
