...
## 비실사 스타일 다양화 (2026-04-29 추가 5종)

기존 실사 페르소나 외에 **비실사 일러스트레이션 페르소나 5종** 추가 검증. 같은 K-뷰티 광고 narrative를 다양한 시각 스타일로 노출해 광고 피로 회피.

| 페르소나 | 스타일 | 톤 | hero 생성 시간 |
|---|---|---|---|
| persona_v22_3d_character | Pixar/Disney 3D CGI | 친근/캐릭터스러운 | 69.3s |
| persona_v23_realistic_elegant | 40대 우아 실사 | 마흔 광채 angle | 30.8s |
| persona_v24_anime_japanese | Studio Ghibli 일본 애니 | 감성/감동 | 168.1s |
| persona_v25_manhwa_sunjeong | 한국 순정만화 | 로맨틱 드라마 | 빠름 |
| persona_v26_us_comic | Spider-Verse 미국 만화 | 슈퍼히어로/자신감 | 36.8s |

**모두 fal-ai/nano-banana-pro 단일 모델 단일 시도로 성공.** 비실사 스타일도 동일 endpoint로 처리 가능 — 별도 Stable Diffusion / Flux / DALL-E 분기 불필요.

**스타일별 angle 차별화**:
- 3D: 과학/성분 (PDRN/비타C 강조)
- 실사 40대: 마흔 광채 (연령대 차별)
- 일본 애니: 감성 변화 (마음까지 가벼워짐)
- 순정만화: 남친 알아챔 (로맨틱 드라마)
- 미국 만화: 슈퍼히어로 자신감 (와 미쳤다 진짜)

**Why 다양화**: 같은 페이스북/틱톡 피드에서 같은 인물이 반복 노출되면 광고 피로 급격히 증가 (CPM↑ CTR↓). 5가지 시각 스타일 + 5가지 angle = 25 조합 가능 → A/B 테스트로 최강 조합 발견.

**How to apply**: 한 캠페인에 4~5 스타일 동시 출고. 같은 narrative arc (노란끼→광채) 유지하면서 angle/톤만 차별. 검증된 hook word 재활용 + 스타일별 감탄사 (와 미쳤다 / 두근두근 / 어 어 어).
