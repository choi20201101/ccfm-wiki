...
> - **[[src-rubyrn-glow-deep-research-success-2026-04-28]] — 1~4단 심화 (광채/PDRN, 화장품 영상기획 표준)** ⭐
> - **[[src-nurse-shoes-2026-04-28]] — §9 변형 모드 (브랜드 시장 단위 + 자사몰 분석 + 진입 의사결정 + 시너지 ROAS, 비-화장품 표준)** ⭐
>
> ⭐ **2026-04-28 업데이트**: 본 플레이북은 이제 **2가지 모드**로 분기.
> - 화장품/소비재 카테고리 → §1-8 (4단 심화, 영상기획 11파일)
> - 비-화장품(패션/가전/푸드 등) 또는 브랜드 진입 검토 → **§9 변형 모드** (Excel 11시트 + Toss UI HTML)
> 시장조사 요청 받으면 사용자 의도(영상기획 vs 진입 의사결정)에 따라 모드 분기.
...
### 9-6. 🏆 성공 케이스 (사용자 승인 표준)

[[sources/src-nurse-shoes-2026-04-28]] — **간호사 신발 55개 + 자사몰 12개 + 착한구두 시너지 3 시나리오**

**사용자 승인 포인트** (2026-04-28):
- 4단 파이프라인 ~30분 완주 (Phase 1 병렬 6분 + Phase 2 API 3분 + Phase 3 자사몰 10분 + Phase 4 작성)
- 산출 5종: 통합 MD + Excel 11시트 + Toss UI HTML 시장조사 + 시너지 MD + Toss UI HTML 시너지
- 검색량 24개월 역산 (DataLab 비율 × SearchAd 절대량) — 본 케이스에서 검증된 공식
- "기존 브랜드 시너지 검토" 옵션 새로 추가 (퍼포먼스 마케팅 관점, ROAS 3 시나리오 비교)

**다음 케이스 진입 시 그대로 따라하기**:
1. 시드 브랜드 2-3개 + 카테고리 확정 (`AskUserQuestion`)
2. 워크폴더 4 서브폴더 (`{reports,data,raw,assets}`)
3. Phase 1 병렬 디스패치 — 발굴 + 페인 (산출 파일 경로 명시, 본문 텍스트 반환 금지)
4. Phase 2 — `fetch_brand_trends.py` 변형 실행 (brands.json 입력)
5. Phase 3 — 핵심 단독 3 + 그룹 3 = 6 에이전트 병렬
6. Phase 4 — 통합 MD → Excel 11시트(generate_excel_report.py) → Toss UI HTML(report.html 템플릿)
7. (옵션) 모브랜드 시너지 — PRO 7/CON 6/3 시나리오 (N/M 비율로 자동 판정)
8. 새 source 페이지로 위키화 (이 페이지를 복제 + 케이스 결과 채워넣기)

**기존 화장품 케이스와 상호보완**:
- [[sources/src-rubyrn-glow-deep-research-success-2026-04-28]] — 화장품, 영상기획 11파일 산출
- [[sources/src-nurse-shoes-2026-04-28]] — **비-화장품(브랜드 시장 단위), 진입 의사결정 11시트**
