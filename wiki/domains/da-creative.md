...
## 광고 아이디어 소싱
- [[domains/todayhumor-mining-playbook]] — **커뮤니티 위트 → 광고 아이디어 1:1 변환** (2026-05-12 피코세라 검증). 추천 N+ 게시물에서 시그니처 표현 추출 → 브랜드 후킹 패턴 A~F 차용 카피 5종 + 레이아웃 + Imagen 프롬프트. 게시물 1개 = 폴더 1개 = 아이디어 1개.
...
## 소재 검수 하네스 (2026-05-14 추가)

생성한 소재를 **점수로 자동 분류**하는 검수 레이어. "잘 뽑힌 소재"를 운이 아니라
같은 기준으로 매번 거르게 만든다.

- **피코세라 1차 하네스** → [[sources/src-picosera-harness-v1-2026-05-14]]
  - 광고 소재 371장을 0~5점 17규칙으로 자동 분류 (4·5점 266장 = `_정리_4점5점/`)
  - 비전 체크리스트 25항목 → 우선순위 규칙표 → 점수·라벨. 비전 판단은 서브에이전트 격리
  - ⚡ **트리거**: "피코세라 하네스 기준으로 필터링해줘" → 그 페이지 §3 절차
- **일반화 스킬 `ad-bdh`** → [[domains/ai-automation]]
  - 루브릭을 `rubric.<버전>.yaml`로 데이터화 — 기준이 바뀌면 코드 수정 없이 v2 추가
  - 1차 결과를 덮어쓰지 않고 버전별 폴더에 분리 저장 (멀티패스 검수)
  - 소재 생성 시마다 hook/수동으로 검수 서브에이전트 자동 디스패치
...
### Gemini Omni (Omni Flash) — v2v 합성 단축 (2026-05-26 추가)
→ [[src-gemini-omni-guide]]

**효과**: 기존 "누끼 이미지 합성 → I2V" 2스텝을 **1스텝**으로 압축.
**대표 적용**: 메라블 루비알엔 손-제품 합성, 슬리퍼→신발 교체, 착한구두 럭셔리 패션 에디토리얼 5변주.
**캐릭터 일관성**: 캐릭터 에셋 + `@태그`로 멀티-씬 동일 인물 유지 → B&A 시리즈/연작 광고에 직결.
**한계 회피**: 한국어 텍스트는 합성 단계 X → 후처리, 스토리보드 해석 실패는 프롬프트에 `스토리보드 기반 [브랜드] [제품] [포맷] 영상 제작` 명시.
